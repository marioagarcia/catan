...
public class LoginView extends OverlayView implements ILoginView {
	...
	private boolean passwordAgainCorrect = false;
	...
	private class RegisterPanel extends JPanel {
		...
		private void initLayout() {
			...
			txtPasswordAgain.addActionListener(actionListener);
			internalInputBox.add(lblUsername);
			internalInputBox.add(txtUsername);
			internalInputBox.add(lblPassword);
			internalInputBox.add(txtPassword);
		...
		private void initEventListeners() {
			...
			TextFieldValidator usernameValidator = new TextFieldValidator(
				...
				public boolean validateContents(String username) {
					...
					if (username.length() < MIN_UNAME_LENGTH
						...
						registerPanel.btnRegister.setEnabled(false);
						return false;
					} else {
					...
					if(isInputValid()){
						registerPanel.btnRegister.setEnabled(true);
					}
					return true;
				}
			};
			...
			TextFieldValidator passAgainValidator = new TextFieldValidator(
				...
				@Override
				public boolean validateContents(String input) {
					if(input.equals(txtPassword.getText())){
						passwordAgainCorrect = true;
						if(isInputValid()){
							registerPanel.btnRegister.setEnabled(true);
						}
						return true;
		...
		public Boolean isInputValid(){
			if(usernameCorrect && passwordCorrect && passwordAgainCorrect){
				return true;
			}else{
				return false;
			}
	...
	public void setRegisterButtonEnabled(Boolean bool){
		registerPanel.btnRegister.setEnabled(bool);
	}
